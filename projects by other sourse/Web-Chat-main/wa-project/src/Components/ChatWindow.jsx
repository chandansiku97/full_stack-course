...
function ChatWindow() {
  ...
  const chatId =
    userData?.id > receiverId
      ...
      useEffect(() => {
        console.log("Receiver ID:", receiverId); // Log the value of receiverId
        
        const getUser = async () => {
          if (!receiverId) {
            console.error("Receiver ID is missing or invalid.");
            return;
          }
    ...
    <div className="h-full w-full bg-gray-50 dark:bg-gray-900 flex flex-col">
        ...
        {msgList?.map((m, index) => (
          <div
            ...
            bg-gray-100 dark:bg-gray-700 text-black dark:text-white 
              data-[sender=true]:ml-auto data-[sender=true]:bg-primary-light dark:data-[sender=true]:bg-primary-dark`}
          >
            <p>{m?.text}</p>
      ...
      <div className="bg-background py-3 px-6 shadow flex items-center gap-6">
        ...
        <input
          ...
          className="w-full py-2 px-4 rounded bg-white dark:bg-gray-800 text-black dark:text-white focus:outline-none"
