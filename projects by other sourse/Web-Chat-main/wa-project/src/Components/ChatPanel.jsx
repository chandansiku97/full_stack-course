...
import { CircleFadingPlusIcon, Loader2Icon, MessageSquare, SearchIcon, UserRoundIcon, SunMoon } from "lucide-react";
...
function ChatPanel() {
    ...
    return (
        <div className={`w-[30vw] min-w-[350px] bg-white dark:bg-gray-900 text-black dark:text-white`}>
            {/* top-bar */}
            <div className="bg-background dark:bg-gray-800 py-2 px-4 border-r border-gray-300 dark:border-gray-700 flex justify-between items-center gap-2">
                <button
            ...
            {/* chat List */}
            {
                isLoading ? <div className="h-full w-full flex justify-center items-center"><Loader2Icon className="w-10 h-10 animate-spin" /> </div> :
                    ...
                    <div className="bg-white dark:bg-gray-900 py-2 px-3">
                        {/* Search Bar  */}
                        <div className="bg-background dark:bg-gray-800  flex items-center gap-4 px-3 py-2 rounded-lg">
                            <SearchIcon className="w-4 h-4" />
                            <input
                                className="bg-background  dark:bg-gray-800 focus-within: focus-within:outline-none"
                                placeholder="Search"
                                value={searchQuery}
                                onChange={(e) => setSearchQuery(e.target.value)}
                            />
                        </div>
                        <div className="py-4 divide-y divide-gray-300 dark:divide-gray-700 h-full max-h-[calc(100vh-152px)] overflow-y-scroll">
